...
export function SocialPostRow({ post }: { post: SocialPost }) {
  ...
  return (
    <li>
      {/* The row itself opens the post. Retry sits outside this button — a
          control inside a control is not a thing a browser will let you
          click. */}
      <button
        type="button"
        onClick={() => setOpen(true)}
        className="flex w-full flex-col gap-1.5 px-(--card-spacing) py-3 text-left transition-colors outline-none hover:bg-muted/60 focus-visible:bg-muted/60"
      >
